...
export function PageHeader({
  ...
  className?: string;
  heading?: string;
  variant?: 'default' | 'blogPost' | 'allCollections' | 'policies';
  [key: string]: any;
}) {
  const variants: Record<string, string> = {
    ...
    policies: 'flex justify-between items-baseline gap-8 py-6 md:py-8 lg:py-12',
  };
  ...
  return (
    <header {...props} className={styles}>
      {heading && (
        <Heading as="h1" width="narrow" size="heading" className="block">
          {heading}
        </Heading>
      )}
      {children}
